...
## Alcance — Dos fases, dos contratos

Este es el punto estructural más importante del contrato:

### Fase 1 — Modernización de la plataforma *(este contrato)*
...
- Las herramientas de migración (framework reutilizable) diseñadas para que los hospitales futuros puedan ser migrados de forma eficiente.
...
### Fase 2 — Implementación por hospital *(contrato aparte, cada vez)*

Cuando el primer hospital esté listo para usar la plataforma con pacientes reales — **incluyendo la institución donde trabaja el fundador** — hace falta una fase de implementación aparte:

- Provisionar un ambiente de Producción dedicado para ese hospital.
- Migrar sus datos existentes, si los tienen (usando las herramientas de migración construidas en la Fase 1).
- Capacitar a su personal.
- Activar los controles del PHI Gate sobre ese ambiente específico.

Esta fase típicamente toma **1 a 3 semanas por hospital**, dependiendo de si tienen datos previos para migrar y la complejidad de su configuración. Se planifica y contrata por separado, por hospital.

---

## Ambientes incluidos en este contrato

Tres ambientes conforman la plataforma completa. **Este contrato cubre dos de ellos**; el tercero se provisiona por separado, por hospital, cuando hace falta.
...
| **Producción (por hospital)** | Se activa cuando un hospital firma y va a usar la plataforma con pacientes reales. Todos los controles HIPAA activos y auditados. | **Datos reales de pacientes — regulados por HIPAA** | ❌ No incluido. Se provisiona por hospital durante una implementación de Fase 2. |
...
| **M3** | **PHI Gate certificado — plataforma lista para onboarding de hospitales** | Fin de Semana 8 | Los 14 controles HIPAA verificados y documentados. Pentest externo aprobado. La plataforma está técnicamente lista para aceptar datos reales de cualquier hospital en una implementación de Fase 2. |
...
**Migración (tooling):** Diseño inicial del framework de scripts de migración (se usa en futuras implementaciones por hospital).
**HIPAA:** Proveedor de pentest externo contratado y agendado; infraestructura de backups automáticos construida.
**Entregable:** Esquema desplegado en el ambiente Demo; framework armado.
...
**Migración (tooling):** Framework de migración documentado para uso en implementaciones por hospital.
...
**Construcción:** Pulido final del Demo; documentación del playbook de implementación por hospital (se usa en Fase 2).
**HIPAA:** Los 14 controles verificados, documentados individualmente, y firmados (incluido el control #14).
**Entregable:** Certificación del PHI Gate. La plataforma está técnicamente lista para aceptar el onboarding del primer hospital (Fase 2).
...
| Med-Rok quiere onboardear un hospital antes de certificar el PHI Gate | Exposición HIPAA | El onboarding de hospitales (Fase 2) sólo comienza después de firmar el hito M3 (Semana 8). |
...
- La **separación entre Fase 1 y Fase 2** está reconocida y aceptada.
